...
export class DrawContext implements Disposable {
  ...
  drawText(key: string, text: string, position: vec3, style?: CSSProperties, mode: "immersive" | "orthographic" = "immersive") {
    ...
    if (mode === "immersive") {
      ...
      container.style.transform = `translate(${(viewPosition[0] / viewPosition[2]) * 50 + 50}vw, ${(-viewPosition[1] / viewPosition[2]) * 50 + 50}vh) translate(-50%, -50%) scale(${scaleByScreen * 100}%)`
      container.style.zIndex = `${Math.floor(-viewPosition[2] * 1000 + 20000)}`
    } else if (mode === "orthographic") {
      const aspect = window.innerWidth / window.innerHeight
      container.style.transform = `translate(${position[0] / aspect * 50 + 50}vw, ${-position[1] * 50 + 50}vh) translate(-50%, -50%) scale(${scaleByScreen * 100}%)`
